...
gg <- ggplot(df, aes(x = length, y = len_per, fill = prevalence)) +
        ...
        ylim(0,100) +
        facet_wrap(~yRNA, nrow = 1) +
        scale_fill_gradientn(colours = c('#289fd6', '#f4ee42', '#f45041'), limits=c(0, 100)) +
        theme_bw() +
        labs(list(title="Length Distribution", x="Read length", y="Percent of total reads")) 
...
df <- unique(df[,c("sample","yRNA","prevalence")])
...
ggplot(df, aes(sample, prevalence, fill = yRNA)) +
  ...
  labs(list(title="yRNA Distribution", x="Sample", y="Percent of total yRNA reads")) +
